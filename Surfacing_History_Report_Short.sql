...
case when trim(s.BULK_SPC_GR_M) is not null then
  case when s.BULK_SPC_GR_M > 100 
      then round(s.BULK_SPC_GR_M*27.00,3)
           else round((s.BULK_SPC_GR_M*62.4)*27.00,3) end
  else null
      end as As_Built_Unit_Weight,
...
e.as_built_specific_gravity,
case when trim(e.as_built_specific_gravity) is not null then
  case when e.as_built_specific_gravity > 100 
      then round(e.as_built_specific_gravity*27.00,3)
           else round((e.as_built_specific_gravity*62.4)*27.00,3) end
  else null
      end as As_Built_Unit_Weight,
...
((c.BULK_SPC_GR_M * 62.4)*27) as As_Built_Unit_Weight,
