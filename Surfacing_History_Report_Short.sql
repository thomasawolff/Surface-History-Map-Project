...
case when trim(s.BULK_SPC_GR_M) is not null then
  case when s.BULK_SPC_GR_M > 120 and s.BULK_SPC_GR_M < 1000 
      then round(s.BULK_SPC_GR_M/62.4,3)
        when s.BULK_SPC_GR_M > 2.25 
          then round(s.BULK_SPC_GR_M,3)
            when s.BULK_SPC_GR_M > 1000 
               then round(s.BULK_SPC_GR_M/1000,3) end
  else null
...
case when trim(s.BULK_SPC_GR_M) is not null then
  case when s.BULK_SPC_GR_M > 120 and s.BULK_SPC_GR_M < 1000 
      then round(s.BULK_SPC_GR_M*27.00,3)
        when s.BULK_SPC_GR_M > 2.25
           then round((s.BULK_SPC_GR_M*62.4)*27.00,3) end
  else null
...
case when t.AVG_DEN is not null and t.AVG_DEN > 1000
  then round(t.AVG_DEN/1000,3)  
      when t.AVG_DEN is not null 
        and t.AVG_DEN > 20.00 and t.AVG_DEN < 120 then null
          when t.AVG_DEN is not null and t.AVG_DEN > 120
            then round(t.AVG_DEN/62.4,3)
  else round(t.AVG_DEN,3)
    end as Design_Bulk_Gravity,
    
case when trim(t.AVG_DEN) is not null 
  and t.AVG_DEN > 120 and t.AVG_DEN < 1000
    then round(t.AVG_DEN*27.00,0)
      when t.AVG_DEN > 1000
        then round(((t.AVG_DEN/1000)*62.4)*27.00,0)
          when t.AVG_DEN > 20 and t.AVG_DEN < 100 then null
  else round(((t.AVG_DEN*62.4))*27.00)
...
case when trim(e.as_built_specific_gravity) is not null then
  case when e.as_built_specific_gravity > 120 and e.as_built_specific_gravity < 1000 
      then round(e.as_built_specific_gravity/62.4,3)
        when e.as_built_specific_gravity > 2.25 
          then round(e.as_built_specific_gravity,3)
            when e.as_built_specific_gravity > 1000 
               then round(e.as_built_specific_gravity/1000,3) end
  else null
      end as As_Built_Bulk_Gravity,
       
case when trim(e.as_built_specific_gravity) is not null then
  case when e.as_built_specific_gravity > 120 and e.as_built_specific_gravity < 1000 
      then round(e.as_built_specific_gravity*27.00,3)
        when e.as_built_specific_gravity > 2.25
           then round((e.as_built_specific_gravity*62.4)*27.00,3) end
  else null
      end as As_Built_Comp_LBS_Per_CY,
...
case when e.design_density is not null 
  and e.design_density > 1000
  then round(e.design_density/1000,3)  
      when e.design_density > 20.00 
        and e.design_density < 120 then null
          when e.design_density > 120
            then round(e.design_density/62.4,3)
  else round(e.design_density,3)
    end as Design_Bulk_Gravity,
    
case when trim(e.design_density) is not null 
  and e.design_density > 120 and e.design_density < 1000
    then round(e.design_density*27.00,0)
      when e.design_density > 1000
        then round(((e.design_density/1000)*62.4)*27.00,0)
          when e.design_density > 20 and e.design_density < 100 then null
   else round(((e.design_density*62.4))*27.00)
       end as Design_Comp_LBS_Per_CY,
...
case when trim(c.BULK_SPC_GR_M) is not null then
  case when c.BULK_SPC_GR_M > 120 and c.BULK_SPC_GR_M < 1000 
      then round(c.BULK_SPC_GR_M/62.4,3)
        when c.BULK_SPC_GR_M > 2.25 
          then round(c.BULK_SPC_GR_M,3)
            when c.BULK_SPC_GR_M > 1000 
               then round(c.BULK_SPC_GR_M/1000,3) end
  else null
      end as As_Built_Bulk_Gravity,
...
case when trim(c.BULK_SPC_GR_M) is not null then
  case when c.BULK_SPC_GR_M > 120 and c.BULK_SPC_GR_M < 1000 
      then round(c.BULK_SPC_GR_M*27.00,3)
        when c.BULK_SPC_GR_M > 2.25
           then round((c.BULK_SPC_GR_M*62.4)*27.00,3) end
  else null
      end as As_Built_Comp_LBS_Per_CY,
