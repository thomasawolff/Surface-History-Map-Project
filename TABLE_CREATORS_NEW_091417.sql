...
create table SUPERPAVE_TABLE as
select t.MIX_ID,t.effdt,t.matl_cd,
t.AIR_VOIDS_P,t.VMA_P,t.VFA_P,t.BULK_SPC_GR_M,
t.ASPH_CEM_T,t.OPT_AC_PCT_TOT_WT,t.ESALS_NBR
from SUPERPAVE_TABLE t
;
alter table SUPERPAVE_TABLE add primary key(mix_id)
;
...
from SUPERPAVE_TABLE t -- Primary key = mix_id
inner join SMGR.T_CONT_MIX_DSN d
on d.mix_id = t.mix_id
inner join SMGR.T_CONT x -- Primary key = cont_id
...
;
alter table CONT_CRIT_DT_TABLE add primary key(cont_id)
